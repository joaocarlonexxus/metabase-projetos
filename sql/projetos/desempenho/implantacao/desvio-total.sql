		...
		(
			CASE
				...
				[[WHEN {{setor}} AND proj.setor IN ('Automação', 'Automação Ágil')
					THEN
						CASE
							WHEN proj.tempo_real_implant_automacao > 0
								AND proj.tempo_estimado_implant_automacao > 0
							THEN 1 ELSE 0
						END]]
				[[WHEN {{setor}} AND proj.setor = 'Elétrica'
					THEN
						CASE
							WHEN proj.tempo_real_implant_eletrica > 0
								AND proj.tempo_estimado_implant_eletrica > 0
							THEN 1 ELSE 0
						END]]
				[[WHEN {{setor}} AND proj.setor = 'Sistemas'
					THEN
						CASE
							WHEN proj.tempo_real_implant_sistemas > 0
								AND proj.tempo_estimado_implant_sistemas > 0
							THEN 1 ELSE 0
						END]]
				ELSE
					CASE
