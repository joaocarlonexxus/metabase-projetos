...
(
    SELECT
        ...
        MAX(
            CASE
                WHEN proj.setor IN ('Automação', 'Automação Ágil') THEN proj.tempo_real_desenv_automacao
                WHEN proj.setor = 'Elétrica' THEN proj.tempo_real_desenv_eletrica
                WHEN proj.setor = 'Sistemas' THEN proj.tempo_real_desenv_sistemas
                ELSE proj.tempo_real_desenv_geral
            END
        ) AS tempo_real,
        MAX(
            CASE
                WHEN proj.setor IN ('Automação', 'Automação Ágil') THEN proj.tempo_estimado_desenv_automacao
                WHEN proj.setor = 'Elétrica' THEN proj.tempo_estimado_desenv_eletrica
                WHEN proj.setor = 'Sistemas' THEN proj.tempo_estimado_desenv_sistemas
    ...
    WHERE
		...
		(
			CASE
				...
				ELSE
					CASE
						WHEN proj.tempo_real_desenv_geral > 0 AND proj.tempo_estimado_desenv_geral > 0
						THEN 1 ELSE 0
					END
			END = 1
		)
