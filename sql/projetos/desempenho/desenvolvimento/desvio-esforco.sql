		...
		(
			CASE
				...
				[[WHEN {{setor}} AND proj.setor IN ('Automação', 'Automação Ágil')
					THEN
						CASE
							WHEN proj.tempo_real_desenv_automacao > 0
								AND proj.tempo_estimado_desenv_automacao > 0
							THEN 1 ELSE 0
						END]]
				[[WHEN {{setor}} AND proj.setor = 'Elétrica'
					THEN
						CASE
							WHEN proj.tempo_real_desenv_eletrica > 0
								AND proj.tempo_estimado_desenv_eletrica > 0
							THEN 1 ELSE 0
						END]]
				[[WHEN {{setor}} AND proj.setor = 'Sistemas'
					THEN
						CASE
							WHEN proj.tempo_real_desenv_sistemas > 0
								AND proj.tempo_estimado_desenv_sistemas > 0
							THEN 1 ELSE 0
						END]]
				ELSE
					CASE
